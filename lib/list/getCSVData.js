...
function getCSVData (item, options) {
	...
	if (options.fields) {
		...
		options.fields.forEach(function (path) {
            ...
            if (field.type === 'list' && path=='contacts'){
            	var retstr = '';
                item[path].forEach(val=>{
                	if (val.email) retstr = retstr+' email:'+val.email;
                    if (val.faxnumber) retstr = retstr+' faxnumber:'+val.faxnumber;
                    if (val.phonenumber) retstr = retstr+' phonenumber:'+val.phonenumber;
                })
                data[path] =  retstr;
                return;
			}
