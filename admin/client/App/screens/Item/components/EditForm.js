...
var EditForm = React.createClass({
    ...
    renderKeyOrId() {
        ...
        var path = "";
        if(list.autokey && list.autokey.path && this.props.data &&  this.props.data[list.autokey.path]){
            path = this.props.data[list.autokey.path];
        }
        var pageUrl = getBaseUrl() + "/" + list.key.toLowerCase() + "/" + path;
        var newslugdisplayvalue = path;

        if (list.nameField && list.autokey && path) {
            return (
                <div className={className}>
                    ...
                    <AltText
                        ...
                        normal={<input ref="keyOrIdInput" onFocus={this.handleKeyFocus} value={newslugdisplayvalue}
                                       className="EditForm__key-or-id__input" readOnly/>}
                        title="Press <alt> to reveal the ID"
                        className="EditForm__key-or-id__field"/>
                    {list.key == 'Page' && <a target="_blank" href={pageUrl}>Link</a>}
                </div>
            );
        } else if (list.autokey && path) {
            return (
                <div className={className}>
                    <span className="EditForm__key-or-id__label">{list.autokey.path}: </span>
                    <div className="EditForm__key-or-id__field">
                        <input ref="keyOrIdInput" onFocus={this.handleKeyFocus}
                               value={path} className="EditForm__key-or-id__input"
                               readOnly/>
                    </div>
                </div>
            );
