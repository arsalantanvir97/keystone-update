...
file.prototype.updateDBRecord = function (item) {
	var newmodelpart = keystone.modelobj.cmsfield.map(function(val){
		if(item.id==val.id){
			val = item;
		}
		return val;
	});
	keystone.modelobj.set('cmsfield',newmodelpart);
	var newmodelpart = keystone.modelobj.toJSON().cmsfield;
	require('dotenv').config();
	const MongoClient = require('mongodb').MongoClient;
	var ObjectId = require('mongodb').ObjectID;
	const url = (process.env.MONGO_URL || 'mongodb://localhost:27017/returning-from-germany');
	MongoClient.connect(url, function (err, db) {
		...
		collection.update(
			{ "_id": new ObjectId(keystone.modelobj.id)}, {$set: {'cmsfield': newmodelpart }},
			function (err, result) {
				db.close();
			}
		);
...
file.prototype.upload = function (item, file, callback) {
	...
	this.storage.uploadFile(file, function (err, result) {
		...
		if (item.constructor.name == "EmbeddedDocument" && keystone.modelobj) {
			...
			keystone.modelobj.set('cmsfield',newmodelpart)
			var newmodeldata = keystone.modelobj.toJSON().cmsfield;
			...
			MongoClient.connect(url, function (err, db) {
				...
				const collection = db.collection('pages');
				collection.update(
							{ "_id": new ObjectId(keystone.modelobj.id)}, {$set: {'cmsfield': newmodeldata}},
							function (err, result) {
								db.close();
							}
						);
